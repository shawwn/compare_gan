...
printf '\033]2;%s\033\\' "bigrun36 ${TPU_NAME}" # set tmux title
export MODEL_DIR="${MODEL_DIR:-gs://dota-euw4a/runs/bigrun36c/}"
...
while true; do
  timeout --signal=SIGKILL 19h python3 wrapper.py compare_gan/main.py --use_tpu --tfds_data_dir 'gs://dota-euw4a/tensorflow_datasets/' --model_dir "${MODEL_DIR}" --gin_config example_configs/biggan_danbooru128.gin "$@" 2>&1 | tee -a logs36.txt
  sleep 30
done
