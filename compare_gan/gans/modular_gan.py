...
class ModularGAN(AbstractGAN):
  ...
  def set_losses(self, category,
    ...
    self.scalar(category, "000_d_prob_real", prob_real)
    self.scalar(category, "001_d_prob_fake", prob_fake)
    self.scalar(category, "002_d_loss_real", d_loss_real)
    self.scalar(category, "003_d_loss_fake", d_loss_fake)
    self.scalar(category, "010_d_loss_without_flooding", self.d_loss)
    self.scalar(category, "011_g_loss_without_flooding", self.g_loss)
    self.flood_loss()
    self.scalar(category, "020_d_loss_without_stop", self.d_loss)
    ...
    if penalty_loss != 0.0:
      self.scalar(category, "030_d_loss_without_penalty", self.d_loss)
      self.scalar(category, "031_d_penalty", penalty_loss)
      self.d_loss += self._lambda * penalty_loss

  def end_losses(self, category):
    self.scalar(category, "100_d_loss_final", self.d_loss)
    self.scalar(category, "101_g_loss_final", self.g_loss)
